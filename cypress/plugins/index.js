...
function determineScreenshotPath(supposedPath) {
    // In Cypress 10, the screenshot folder is non-determinisitic
    // https://github.com/cypress-io/cypress/pull/19319
    const filename = path.basename(supposedPath);
    const filenameNoExt = filename.split('.')[0];
    const folder = path.dirname(supposedPath);
    ...
    function testPath(attempt) {
        ...
        const tryParent = parentFolder + '/' + filenameNoExt + attemptSuffix;

        if (fs.existsSync(tryPath)) return tryPath;
        if (fs.existsSync(tryParent)) return tryParent;
